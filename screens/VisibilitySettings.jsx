    ...
    if (docSnap.exists) {
      ...
      querySnapshot.forEach((doc) => {
        if (friendUids.includes(doc.id)) {
          this.friends.push({uid: doc.id, name: doc.data().name, email: doc.data().email});
        }
      });
    }
  ...
  ItemRender = ({ friend }) => {
    return <View style={styles.item}>
      ...
      {this.state.visibility.friends_except.includes(friend.uid)?
        <Ionicons style={styles.checkBoxIcon} name="checkbox" size={24} onPress={() => this.uncheckFriendOnPress(friend)} />:
        <Ionicons style={styles.checkBoxOutlineIcon} name="square-outline" size={24} onPress={() => this.checkFriendOnPress(friend)} />
      }
    </View>
  }
      ...
      <View style={styles.optionsContainer}>
        <View style={styles.optionRow}>
          ...
          {this.state.visibility.mode == "only_me"?
            <Ionicons style={styles.checkCircleIcon} name="checkmark-circle" size={24} onPress={() => this.selectOnlyMeOnPress} /> :
            <Ionicons style={styles.checkCircleOutlineIcon} name="ellipse-outline" size={24} onPress={this.selectOnlyMeOnPress} />}
        </View>
        <View style={styles.divider} />
        <View style={styles.optionRow}>
          <Text style={styles.optionText}>My Friends</Text>
          {this.state.visibility.mode == "friends"?
            <Ionicons style={styles.checkCircleIcon} name="checkmark-circle" size={24} onPress={this.selectFriendsOnPress} /> :
            <Ionicons style={styles.checkCircleOutlineIcon} name="ellipse-outline" size={24} onPress={this.selectFriendsOnPress} />}
        </View>
        <View style={styles.divider} />
        <View style={styles.optionRow}>
          <Text style={styles.optionText}>My Friends, Except...</Text>
          {this.state.visibility.mode == "friends_except"?
            <Ionicons style={styles.checkCircleIcon} name="checkmark-circle" size={24} onPress={this.selectFriendsExceptOnPress} /> :
            <Ionicons style={styles.checkCircleOutlineIcon} name="ellipse-outline" size={24} onPress={this.selectFriendsExceptOnPress} />}
        </View>
        {this.state.visibility.mode == "friends_except"?
          <FlatList
            data={this.friends}
