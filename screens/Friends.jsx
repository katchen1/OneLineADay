...
class FriendsScreen extends React.Component {
    ...
    return (
      <View style={styles.item}>
        ...
        <Ionicons
          ...
          name="person-remove"
          size={28}
          onPress={() => this.removeFriendOnPress(friend)}
        />
      </View>
    );
  };
  ...
  friendQueryOnChange = (text) => {
    ...
    if (text) {
      ...
      this.setState({
        filteredFriends: this.state.allUsers.filter(
          (user) =>
            user.email.search(regex) >= 0 || user.name.search(regex) >= 0
        ),
      });
    } else {
              ...
              <View style={styles.searchItem}>
                ...
                {this.state.friends.some((e) => e.email === item.email) ? (
                  <Ionicons
                    ...
                    name="person-remove"
                    size={28}
                    onPress={() => this.removeFriendOnPress(item)}
                  />
                ) : (
                  <Ionicons
                    style={styles.removeButton}
                    name="person-add"
                    size={28}
                    onPress={() => this.addFriendOnPress(item)}
                  />
                )}
              </View>
            ),
